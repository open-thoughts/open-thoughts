...
if __name__ == "__main__":
    ...
    for subset in ["puzzle", "science", "math", "code"]:
        ds = load_dataset(
            f"{org}/open-thoughts-{subset}{'-dry-run' if args.dry_run else ''}",
            split="train",
        )
    ...
    unverified_mix.push_to_hub(
        f"{org}/open-thoughts-unverified-mix{'-dry-run' if args.dry_run else ''}",
        private=os.environ.get("HF_PRIVATE"),
    )

    verified_mix = concatenate_datasets(verified_mix_ds)
    verified_mix.push_to_hub(
        f"{org}/open-thoughts-verified-mix{'-dry-run' if args.dry_run else ''}",
        private=os.environ.get("HF_PRIVATE"),
    )
